...
class ClientGateway(rpc.ClientGatewayServicer):
    ...
    def GetNext(self, request, context):
        print(f"[{self.node_id}] ⏭️  GetNext: {request.request_id}")
    ...
    def RequestOnce(self, request, context):
        ...
        return pb.AggregatedResult(
            ...
            total_rows=42,  # Using proper int type
            total_bytes=1024  # Using proper int type
        )
...
def main():
    ...
    if args.config:
        try:
            ...
            print(f"[{args.node_id}] ⚠️  Could not load config: {e}")
