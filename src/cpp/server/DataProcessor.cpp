...
bool DataProcessor::LoadDataset() {
    ...
    if (!file.is_open()) {
        std::cerr << "[DataProcessor] ERROR: Cannot open dataset: " << dataset_path_ << std::endl;
        return false;
    }
    
    std::cout << "[DataProcessor] Loading dataset: " << dataset_path_ << std::endl;
...
std::vector<CSVRow> DataProcessor::GetChunk(size_t start_idx, size_t count) {
    ...
    if (start_idx >= data_.size()) {
        std::cerr << "[DataProcessor] WARNING: start_idx " << start_idx 
    ...
    std::cout << "[DataProcessor] Retrieved chunk: start=" << start_idx 
...
std::string DataProcessor::ProcessChunk(const std::vector<CSVRow>& chunk, const std::string& filter_column, const std::string& filter_value) {
    ...
    std::cout << "[DataProcessor] Processed " << processed << " rows";
    if (!filter_column.empty()) {
        std::cout << " (filtered by: " << filter_column << "=" << filter_value << ")";
    }
    std::cout << std::endl;
